...
<head>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .btn-unban:hover { background: rgba(255,255,255,0.2); }
...
<script>
    ...
    // ═══════════════════════════════════════
    // 🟠 ENHANCED ADMIN PANEL LOADER
    // ═══════════════════════════════════════
    async function loadAdminPanel() {
        const gridContainer = document.querySelector('.grid');
        if (!gridContainer) return;

        // إزالة لوحة الأدمن القديمة إن وجدت
        ...
        const totalOnline = Object.values(allUsers).filter(u => u.lastSeen && (Date.now() - u.lastSeen < 120000)).length; // نشط خلال دقيقتين
        ...
        adminDiv.innerHTML = `
            ...
            <div class="admin-stats-grid">
                <div class="stat-card">
                    ...
                    <div class="stat-info"><h4>المستخدمين</h4><span>${totalUsers}</span></div>
                </div>
                <div class="stat-card">
                    <div class="stat-icon" style="background: linear-gradient(135deg, #22c55e, #16a34a);"><i class="fas fa-wifi"></i></div>
                    <div class="stat-info"><h4>متصلين</h4><span>${totalOnline}</span></div>
                </div>
                <div class="stat-card">
                    <div class="stat-icon" style="background: linear-gradient(135deg, #f59e0b, #d97706);"><i class="fas fa-video"></i></div>
                    <div class="stat-info"><h4>فيديوهات</h4><span>${totalVideos}</span></div>
                </div>
                <div class="stat-card">
                    <div class="stat-icon" style="background: linear-gradient(135deg, #fb923c, #f97316);"><i class="fas fa-check-circle"></i></div>
                    <div class="stat-info"><h4>موثقين</h4><span>${totalVerified}</span></div>
                </div>
                <div class="stat-card" style="grid-column: span 2;">
                    <div class="stat-icon" style="background: linear-gradient(135deg, #ef4444, #dc2626);"><i class="fas fa-ban"></i></div>
                    <div class="stat-info"><h4>محظورين</h4><span>${totalBanned}</span></div>
                </div>
            </div>

            <div class="admin-user-list-header">
                <span>📋 قائمة المستخدمين (أحدث 15)</span>
                <span style="font-size: 11px;">${totalUsers} إجمالي</span>
            </div>
    ...
    async function loadAdminUsersList() {
        ...
        // ترتيب المستخدمين من الأحدث للأقدم
        const usersArray = Object.entries(allUsers)
            .sort(([, a], [, b]) => (b.createdAt || 0) - (a.createdAt || 0))
            .slice(0, 15);

        if (!usersArray.length) {
            listContainer.innerHTML = '<div style="text-align:center;opacity:0.5;padding:20px">لا يوجد مستخدمون</div>';
            return;
        }
        ...
        listContainer.innerHTML = usersArray.map(([id, u]) => {
            ...
            // تحديد حالة الزر بناءً على المستخدم
            let actionBtn = '';
            if (isBanned) {
                actionBtn = `<button class="admin-btn btn-unban" onclick="toggleBanUser('${id}')"><i class="fas fa-undo"></i> إلغاء الحظر</button>`;
            } else {
                actionBtn = `
                    ...
                    <button class="admin-btn btn-unban" onclick="toggleBanUser('${id}')"><i class="fas fa-ban"></i> حظر</button>
                `;
            }

            return `
                <div class="admin-user-item" onclick="if(event.target.tagName !== 'BUTTON' && event.target.tagName !== 'I') openUserProfile('${id}')" style="cursor: pointer;">
                    <div class="admin-user-info">
                        <div class="admin-avatar"><img src="${avatar}" alt="avatar"></div>
                        <div class="admin-user-details">
                            <h4>@${username} ${isVerified ? '<span class="badge-verified">✅</span>' : ''}</h4>
                            <p>${email}</p>
                        </div>
                    </div>
    ...
    window.toggleBanUser = async function(id) {
        ...
        await db.ref('users/' + id).update({ 
            banned: newState,
            bannedAt: newState ? Date.now() : null,
            bannedBy: newState ? currentUser.uid : null
        });
    ...
    // لجعل الدالة window.openUserProfile تنادي الدالة الصحيحة
    ...
    console.log('🟠 Orange Crystal Profile v3 Ready ✨');
</script>
</body>
</html>"""
